...
Git 网址 https://gitee.com/itcastitheima/meiduo_mall_42.git
tel: 18310820688  手机和微信
...
SECRET_KEY = 'nx6zqvx4om^4j*i5&@n72l)umkk+0y&c%+nrk%z@pk8cl)h=6('
...
INSTALLED_APPS = [
    ...
    # 'users',
    'apps.users',  #因为我们的子应用已经放到apps的包中,所以要添加apps.xxx
...
TEMPLATES = [
    {
        ...
        'OPTIONS': {
            'environment':
            'utils.jinja2_enviroment.environment',  #指定环境
            'context_processors': [
...
HAYSTACK_CONNECTIONS = {
    'default': {
        ...
        'URL':
        'http://192.168.229.148:9200/',  # Elasticsearch服务器ip地址，端口号固定为9200
...
APP_PRIVATE_KEY_PATH = os.path.join(BASE_DIR,
                                    'apps/pay/keys/app_private_key.pem')
ALIPAY_PUBLIC_KEY_PATH = os.path.join(BASE_DIR,
                                      'apps/pay/keys/alipay_public_key.pem')
